get_fft2 <- function(stim_image_gray, scr.ppd, var.name="", 
  ...
  # embed the stimulus in zero pads?
  if (!is.na(pad_image_size)) {
    stim_pad_mat <- matrix(0, nrow = pad_image_size, ncol = pad_image_size)
    stim_pad_mat[1:nrow(stim_image_gray), 1:ncol(stim_image_gray)] <- stim_image_gray
    stim_image_gray <- stim_pad_mat
  }
  # perform FFT
  if (use_torch) {
