...
Note that these are the normal Gabors that we will not use! We will use log-Gabors instead (see further below).
```{r}
...
Heikos RFs get insanely huge at low SFs, if the bandwidth is kept constant. 
...
Anderson & Burr (1989) describe how SF-bandwidth changes at low spatial frequencies, we'll use that increase the bandwidth as SFs get smaller. 
```{r}
...
Now create new set of log Gabor filters with increased bandwidth at low SFs:
```{r}
...
```{r,echo=FALSE}
use_this_stim <- all_noise_patches[[1]]
use_half_prec <- FALSE
use_debug_mode <- FALSE
use_final_maps <- TRUE
use_full_output <- FALSE
# plot stim real quick
plot_heatmap(use_this_stim, use_grayscale = TRUE)
# normal Gabors with normalization pool
v1_output <- v1(stim_mat = use_this_stim, # the noise patch as a matrix
                ...
                output_full_sequences = use_full_output, # TRUE: returns complete sequences. WARNING: this is memory-intense
                signal_x_range = NULL, signal_y_range = NULL, signal_t_range = NULL, # the min and max of temporal scale
                no_CUDA = FALSE, # set to TRUE if you want to use CPU instead of GPU (but torch)
                use_half_precision = use_half_prec, # if a GPU is available, use half-precision?
                spatial_resolution = spatial_resolution, # spatial resolution of processing function in dva
                temporal_resolution = temporal_resolution, # temporal resolution of processing function in milliseconds
                use_normalization_pool = TRUE, # use normalization pool?
                debug_mode = use_debug_mode, # shows output at every step
                show_final_maps = use_final_maps,  # shows all resulting 2D maps at the end
...
Test the capability to spatially downsample with a reduced number of filters here:
```{r}
...
Aggregate saccade landing positions...
Because what is the decision-relevant variable? - the post-saccadic target position.
...
For the sensorimotor contingency, however, it may be the entire trajectory. 
We will thus approximate sensory consequences of the "normal" saccade.
...
From this we will be able to read out the mean position and covariance matrix at each time point.

Load all output rda files and make spatial aggregates. 
We'll also run the spatial localization model...
...
Now there's a theory. How well different conditions can be separated determines the classification accuracy. 
