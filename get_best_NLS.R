...
# Updated: Parallel version with foreach, without the full parameter list
# by Richard Schweitzer
get_best_NLS_foreach <- function(cl=NULL, # NULL if not parallelized, otherwise cluster supplied
                                 df, model_form, model_control,
                                 start_params_low, start_params_high, 
                                 start_params_n, 
                                 absolute_lowest, 
                                 use_robust=FALSE, debug_mode=FALSE) {
  ...
  if (is.null(cl)) { # simple foreach
    `%dofun%` <- `%do%` 
  } else { # parallelized foreach, we assume a cluster has been supplied
    require(doParallel)
    `%dofun%` <- `%dopar%` 
  }
  # check whether params_low and _high are correctly specified
  ...
  # run through all starting parameter combinations
  model_list <- foreach(r = as.integer(1:nrow(start_params)), .verbose = FALSE, .errorhandling = "stop",
                        .packages = c("data.table", "assertthat", "robustbase", "minpack.lm", "pracma"),
                        .export = c("exp_damp_sin", "exp_damp_sin_fun", "bergen_wilson", "bergen_wilson_fun", 
    ...
    tryCatch({
      ...
      if (debug_mode) {
        print(as.numeric(coef(this_fit)))
        print(paste("RSS:", sum(resid(this_fit)^2)))              
      }
    }, error = function(ex) {
      if (debug_mode) {
        print(ex)
  ...
  # get best BIC from model list
  all_ssq <- lapply(X = model_list, 
                    FUN = function(x) { 
                      if (!is.null(x)) {
                        res <- sum(resid(x)^2) 
                      } else {
  ...
  all_ssq <- as.numeric(unlist(all_ssq))
  if (debug_mode) { 
    print("All sum of squares:")
    print(all_ssq) 
  }
  best_index <- which(all_ssq==min(all_ssq, na.rm = TRUE))[1]
  # return values
  if (length(best_index)==1) {
  ...
  } else { # none found - that means all NA
    best_model <- NULL
    if (debug_mode) {
      print("best_model = NULL")
    }
  }
  return(best_model)
}
